...
Go:
	...
	move $a1, $s1
	jal pixelLocation
	li $s5, 0xf0000000		#zet in $t1 de kleur (zwart)
	sw	$s5,($v0) 		#s5 = kleur; $v0 = adres
...
left:
	...
	subi $a2,$s0, 1		#$a2 (niuwe pos x) = huidige x - 1
	move $a3,$s1		#$a3 (niuwe pos y) = huidige y
	jal updatePossition
	

	move $s0, $v0		#niuwe pos x -> $s0
	move $s1, $v1		#niuwe pos y -> $s1
	
	move $a0, $s0		#$a0 (huidige pos) = $s0 (nieuw pos)
	move $a1, $s1		#$a0 (huidige pos) = $s1 (nieuw pos)
...
delay:
	sw	$fp, 0($sp)	# push oude frame pointer
	move	$fp, $sp	# frame pointer wijst nu naar bovenaan de stack
	subu	$sp, $sp, 16	# resserveer 32 bytes
	sw	$ra, -4($fp)	# slaag het return adress op op de stack
	sw	$a0, -8($fp)	# slaag het return adress op op de stack
	sw	$v0, -12($fp)	# slaag het return adress op op de stack
	
	li $a0, 200
	li $v0, 32
	syscall
    	
    	lw	$v0, -12($fp)    # zet het return adres terug
    	lw	$a0, -8($fp)    # zet het return adres terug
	lw	$ra, -4($fp)    # zet het return adres terug
	move	$sp, $fp        # 
	lw	$fp, ($sp)	# zet oude frame pointer terug
	jr	$ra		# ga terug naar de aanroeper
